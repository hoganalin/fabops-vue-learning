<!--
FabOps 適配說明（本段為專案增補，其餘內容取自
VoltAgent/awesome-design-md 的 linear.app/DESIGN.md）

1. 字體:Linear Display/Text 不可得,全域一律用 Inter(既有 font stack)。
2. 狀態色擴充:原檔僅有 semantic-success;工廠監控台需要四級狀態色,
   已於深色表面 #0f1011 通過 CVD/對比驗證(dataviz validate_palette):
     status-good:     #27a644
     status-warn:     #bd8b0a
     status-critical: #e5484d
     status-neutral:  #8a8f98   (中性灰,pill 皆附文字標籤,不單靠顏色)
3. 圖表色板(深色表面驗證通過):
     series-1(Lithography A): #5e6ad2   series-2(SMT Line 2): #c56d2b
4. token → CSS 變數對應(src/style.css 的 :root):
     canvas→--canvas  surface-1→--surface-1  surface-2→--surface-2
     ink→--ink  ink-muted→--ink-muted  ink-subtle→--ink-subtle
     hairline→--hairline  primary→--accent  primary-hover→--accent-hover
     rounded sm/md/lg→--r-sm/--r-md/--r-lg
5. 之後任何 UI 修改,顏色一律取自上述變數,不得寫死 hex。
-->
---
version: alpha
